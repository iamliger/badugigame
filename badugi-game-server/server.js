...
io.on('connection', (socket) => {
    ...
    socket.on('createRoom', (roomData, callback) => {
        ...
        const newRoom = {
            ...
            gameRoundName: '대기 중', // 초기화
            currentPhase: 'waiting' // 초기 페이즈
        };
    ...
    socket.on('joinRoom', (data, callback) => { // data 객체로 roomId와 password를 받음
        ...
        const player = {
            ...
            status: 'waiting',
            bestHand: null,
            canExchange: true, // 초기화
            hasActedInBettingRound: false, // 초기화
        };
    ...
    socket.on('leaveRoom', (roomIdToLeave, callback) => {
        ...
        if (room.status === 'playing') {
            ...
            return callback({ success: false, message: '게임이 진행 중이므로 종료 후 나갈 수 있습니다.' });
        }
    ...
    socket.on('cancelLeaveRoom', (roomIdToCancel, callback) => {
        ...
        if (player && room.status === 'playing' && player.leaveReserved) {
            ...
            callback({ success: false, message: '게임 중인 방에서 예약된 퇴장만 취소할 수 있습니다.' });
        }
    });
